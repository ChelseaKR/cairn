...
cairn/                  the package
  ...
  ui/contrast.py        the page's color pairs, read from the stylesheet
...
tests/browser/          Chromium behavior checks — outside the core dev path
...
passage-level analog of the document-title weight: `## Who is eligible` is
...
**Measured, not only asserted — and the assertion has a size.** `benchmark_index.py`
(dev-only, stdlib, not part of the runtime or gated in CI) generates a
deterministic synthetic corpus at a chosen size and times build, read, and
query. Run 2026-08-22: at the demo corpus's scale (10s of documents) query time
is low single-digit milliseconds; at 100 documents / 400 passages, ~44ms median;
at 1,000 documents / 4,000 passages, ~438ms; at 5,000 documents / 20,000
passages, ~2.2s. Scoring is a linear scan of every passage per query — there is
no index structure that skips passages a query cannot match — so the growth is
linear in passage count, and the "milliseconds" claim is accurate for a laptop
demo and stops being accurate somewhere between a few hundred and a few thousand
documents, depending what an operator considers acceptable query latency. If a
real deployment's corpus grows into that range, the concrete fix sketched but
not built is precomputing per-passage vector norms into the index file (an
additive field, a format-version bump like the fingerprint's), which would cut
the per-passage work in the hot loop without changing a score; a real
inverted-index structure would go further but is a larger change to the same
scoring code path. Neither is built, because nothing in this project's own
demo corpus, or the audited evidence, currently needs it — this section exists
so that the "milliseconds" claim has a boundary attached to it instead of
being asserted forever unchecked.
...
language's passages: it is how Cairn gets stopword behavior without shipping
...
| `lint.readability` | empty | per-language readability formula overrides. Built in: `en` Flesch-Kincaid grade level, `es` Crawford grade level. A language with no formula reports `n/a` rather than a grade computed by coefficients fitted on a different language; declaring one here makes that the operator's stated judgment |
...
including the ones blocked on a person, a license, or an upstream project
...
- **The audit scores a correct cross-language answer as a failure, and there
  ...
  - *Accept that the path is audited by exactly one item.* Taken. One item is
    ...
    the milestone it replaced — three paragraphs of README about behavior no
...
- **No manual screen-reader pass.** The browser checks verify the plumbing a
  ...
  it turns the automated suite's own behavioral contracts into an hour-long
...
- **Three live regions with a strict division of labor.** A polite `log`
...
- **A grounded answer can be copied out of the page with its citations
  ...
  interaction (labeled by `aria-label`, not a `<label for>`, so no `id` is
...
**The rule set is a judgment, so it is pinned like one.** This repository
...
"31/31 behavior checks passed" in exactly the green a full run prints. The
...
behavior this project has shipped from the start, and `SECURITY.md` states
...
else about `cairn serve`'s behavior is config-driven by design — but a
...
sitting in front of this server cannot serve one origin's CORS-flavored
...
- **Bumping the pin is a reviewed diff, and the second bump proved it.** Both
  ...
  and even the run id was unchanged, which is what a behavior-neutral upgrade
...
- **The ratchet worked one way, and now works both.** The first version failed
  ...
  deliberateness is not. A rise is labeled `IMPROVEMENT` and a fall
...
theorized. The universe comes from the harness now — a suite it implements is
...
reason: rather than an Arabic word list, it now recognizes Arabic **by
...
For a full milestone the audit had nothing to say about the behavior this
...
the correct behavior and translating a policy statement would make it
...
which is a judgment Cairn has no signal for. `what about a household of four
...
that generalizes: an escalation cannot be pressed past its first turn
...
`TestTheHoleThatIsLeft` records the behavior the way `ck-015` and `ck-022`
