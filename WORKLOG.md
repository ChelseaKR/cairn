...
- 2026-08-28 — Session 21b (AI implementation session). Input: review of
  ...
  dense=-1.0` for every merged candidate, all 807 tests passed. Both
  ...
  it. `transit-pass-en#4` is that passage. A merge taking the best
  lexical rather than the winner's passes all 807 of session 21's tests
  and fails this one by name. `cairn/query.py` is still untouched.
  `make verify`: ruff, mypy --strict, 808 tests, 92% branch coverage.
