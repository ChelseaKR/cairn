...
evidence and, separately, against the running server. 851 tests plus
...
a code change. For what real content actually looks like once imported, see
[`corpus/pilot-usagov/`](corpus/pilot-usagov/) — six real, currently-published
usa.gov pages (`synthetic: false`, sourced and dated in each file's front
matter) and [`docs/pilot-usagov.md`](docs/pilot-usagov.md), the measured
write-up of what importing and calibrating against them actually found.
The next pilot — federal program owners, California agencies, and a county,
assembled per county and asked questions nobody wrote for Cairn — is under
way at [`docs/pilot-ca.md`](docs/pilot-ca.md), with its decision gate
written down before its first number.
...
That default — loopback, no auth, no rate limit — is what `SECURITY.md`
documents as the boundary. An operator who needs to reach it from more than
one machine can opt into bearer-token auth and a per-client rate limit
(`--auth-token`/`CAIRN_AUTH_TOKEN`, `--rate-limit`) without changing anything
about the default path; see [`docs/deployment.md`](docs/deployment.md) for
what that does and does not cover. An agency that wants Cairn reachable
*from* its own site — an iframe, or a fetch to the JSON API — can opt into
that too (`--allow-embed`, `--cors-origin`), independently of each other and
of the deployment flags above; see
...
[`docs/screen-reader-test-script.md`](docs/screen-reader-test-script.md) is a
structured, task-by-task script for whoever runs that session, so it takes an
hour rather than a day of improvising — the script itself is not that session.
...
| Code Quality | Applies — one limit is configured rather than enforced, deliberately, and it is now counted rather than remembered. `make verify` is the local gate: `uv lock --check`, ruff, **mypy in `--strict` mode**, and the suite under coverage with an 85% branch floor against 92% measured. Strict mode reported 44 findings until 2026-08-27; the findings were closed (issues #34-#37, all mechanical) rather than the level claimed, and the check was switched on, because a tree that reports zero with the check off reports one on the next pull request and nothing says so. **The complexity limit of 10 is configured and the `C90` rule is not switched on,** because 5 functions are over it, down from twelve. That number was published as eight while ruff reported twelve — the pilot tooling added four and nothing recompiled a hand-kept list — so the inventory now lives in `tests/test_code_quality.py` and ruff is what confirms it. The seven at 11 and 12 came out by extraction with no behaviour change (`audit_guard.py`'s terminal output checked byte-for-byte either side); the five left are two request-handler closures, a measured ranking algorithm, and two HTML parser state machines, which are a different task. See [docs/roadmap.md](docs/roadmap.md) phase 4 and issues [#42](https://github.com/ChelseaKR/cairn/issues/42) and [#43](https://github.com/ChelseaKR/cairn/issues/43). | `Makefile`, `pyproject.toml`, `tests/test_code_quality.py`, `uv.lock`, `.python-version` |
