...
server. 358 tests plus 63 browser behaviour checks, standard library only,
offline.
...
| Internationalization | Applies — three languages ship (`en`, `es`, `ar`), one of them right to left, with script-aware tokenizing, bidi isolation, and a language selector that mirrors the whole layout. **Not written:** there is no `docs/I18N.md` declaring the scope and the flip conditions, so what is committed to beyond these three languages is undeclared. | `cairn/language.py`, `cairn/messages.py`, `tests/test_multilingual.py` |
