...
| Code Quality | Applies — one limit is configured rather than enforced, deliberately, and it is now counted rather than remembered. `make verify` is the local gate: `uv lock --check`, ruff, **mypy in `--strict` mode**, and the suite under coverage with an 85% branch floor against 92% measured. Strict mode reported 44 findings until 2026-08-27; the findings were closed (issues #34-#37, all mechanical) rather than the level claimed, and the check was switched on, because a tree that reports zero with the check off reports one on the next pull request and nothing says so. **The complexity limit of 10 is configured and the `C90` rule is not switched on,** because 12 functions are over it. That number was published as eight for five days while ruff reported twelve — the pilot tooling added four and nothing recompiled a hand-kept list — so the inventory now lives in `tests/test_code_quality.py` and ruff is what confirms it. Closing it is a refactor rather than a setting; see [docs/roadmap.md](docs/roadmap.md) phases 3 and 4 and the [`good first issue`](https://github.com/ChelseaKR/cairn/labels/good%20first%20issue) label. | `Makefile`, `pyproject.toml`, `tests/test_code_quality.py`, `uv.lock`, `.python-version` |
